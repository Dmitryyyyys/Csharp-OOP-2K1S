...
void Ping(const char* cHost, unsigned int Timeout, unsigned int RequestCount)
{
    ...
    PICMP_ECHO_REPLY pIpe = (PICMP_ECHO_REPLY)GlobalAlloc(GPTR, sizeof(ICMP_ECHO_REPLY) + sizeof(SendData));
    GlobalLock(pIpe);
    ...
    IP_OPTION_INFORMATION option = { 128, 0, 0, 0, 0 };
...
int main()
{
    ...
    {
        cin.clear();
        cout << "Enter ip adress: "; cin >> ipAdress;

        cout << "\n";
        cin.clear();
        Ping(ipAdress, 1000, 10);

    retry:

        cout << "Would you like to ping again? (y - yes, n - no)\nEnter: "; cin >> respond;
